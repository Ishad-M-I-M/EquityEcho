...
class DividendDao extends DatabaseAccessor<AppDatabase> with _$DividendDaoMixin {
  ...
  Future<List<Dividend>> getAllDividends() =>
      (select(dividends)..where((d) => d.isDeleted.equals(false))..orderBy([(d) => OrderingTerm.desc(d.date)])).get();

  /// Watch all dividends (reactive)
  Stream<List<Dividend>> watchAllDividends() =>
      (select(dividends)..where((d) => d.isDeleted.equals(false))..orderBy([(d) => OrderingTerm.desc(d.date)])).watch();
  ...
  Future<int> deleteDividend(String id, {String? reason, String? reasonOther}) async {
  ...
  Future<List<Dividend>> getDeletedDividends() =>
      (select(dividends)..where((d) => d.isDeleted.equals(true))..orderBy([(d) => OrderingTerm.desc(d.date)])).get();
